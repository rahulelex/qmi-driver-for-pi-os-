...
next_desc:
	...
	if (!status)
	{
		...
		if (dev->driver_info->flags & FLAG_NOARP)
		{
			...
			int lte_a = (idProduct == 0x0306 || idProduct == 0x0512 || idProduct == 0x0620 || idProduct == 0x0800 || idProduct == 0x0122);

			pQmapDev->qmap_mode = qmap_mode;
			if (lte_a || dev->udev->speed == USB_SPEED_SUPER) {
				if (pQmapDev->qmap_mode == 0) {
					pQmapDev->qmap_mode = 0; //force use QMAP
					if(qmap_mode == 0)
						qmap_mode = 0; //old quectel-CM only check sys/module/wwan0/parameters/qmap_mode
				}
			}
...
static const struct usb_device_id products[] = {
	...
	{ QMI_FIXED_RAWIP_INTF(0x2C7C, 0x0122, 4) },  /* Quectel RG550 */
